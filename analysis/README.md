...
```
### Table 8 aggregate reduction-quality summary

The aggregate reduction-quality values reported in DKE Table 8 are generated by:

```bash
python analysis/scripts/make_dke_table8_aggregate_reduction_quality.py
```
...
The script filters `baseline = schema_lens`, `run_phase = hot`, and `availability_status = available`. It computes the table from the already generated benchmark artifact without rerunning MongoDB.

Metrics follow the DKE paper definitions: `#cases` is the number of hot query-scale cases, `avg. |A|` is the average number of selected SchemaLens configurations, `avg. DSR = 1 - |A|/10`, `Top-1`, `Top-3`, and `Near-best` are preservation counts, and regret is computed from the artifact column `relative_regret`. The controlled template-space denominator is fixed at 10, corresponding to `G0--G9`.

Current generated values are:

```text
IMDb:     30 cases, avg |A| 7.00, avg DSR 30.0%, Top-1 19/30, Top-3 30/30, Near-best 25/30, mean regret 0.039, max regret 0.454.
FIBEN:    30 cases, avg |A| 5.00, avg DSR 50.0%, Top-1 25/30, Top-3 30/30, Near-best 26/30, mean regret 0.080, max regret 1.390.
LDBC SNB: 66 cases, avg |A| 2.86, avg DSR 71.4%, Top-1 63/66, Top-3 66/66, Near-best 66/66, mean regret 0.001, max regret 0.042.
All:     126 cases, avg |A| 4.36, avg DSR 56.4%, Top-1 107/126, Top-3 126/126, Near-best 117/126, mean regret 0.029, max regret 1.390.
```


### Table 12 baseline-policy examples

The baseline-policy examples reported in DKE Table 12 are generated by:

```bash
python analysis/scripts/make_dke_table12_baseline_policies.py
```
...
The script filters `run_phase = hot` and `availability_status = available`. It then extracts the real candidate sets selected by each baseline for representative cases discussed in the paper: IMDb QG9, FIBEN Q2, and LDBC IC7.

The candidate sets are not manually hard-coded in the table. Only the representative case identifiers and the textual descriptions of each baseline policy are fixed in the script. The selected configuration sets, such as `G0|G3|G6|G7` or `G1|G3|G5|G7|G9`, are read directly from `baseline_performance_by_case.csv`.

The companion file `dke_table12_baseline_policy_sets_summary.csv` provides a complete audit of all candidate sets observed by baseline and dataset, while `dke_table12_baseline_policy_examples.tex` provides the compact paper-facing version.
