...
def build_params(db) -> Dict[str, Any]:
    ...
    if not company:
        company = first_doc(db, "lmm_corporation")

    person = first_doc(db, "lmm_person")
    account = first_doc(db, "lmm_financial_service_account")
    listed_security = first_doc(db, "lmm_listed_security")
    transaction = first_doc(db, "lmm_transaction")
    ...
    return {
        ...
        "listed_security_id": listed_security.get("LISTEDSECURITYID") or transaction.get("REFERSTO"),
        "transaction_refersto": transaction.get("REFERSTO"),
...
def build_queries(params: Dict[str, Any], result_limit: Optional[int]) -> List[Dict[str, Any]]:
    ...
    queries.append({
        ...
        "pipeline": maybe_limit([
            ...
            {"$match": {"financial_service_account.transaction.REFERSTO": security_id}},
            {"$group": {
                "_id": "$PERSONID",
                "buy_total": {
                    "$sum": {
                        "$cond": [
                            {"$eq": ["$financial_service_account.transaction.TRANSACTIONTYPE", buy_type]},
                ...
                "sell_total": {
                    "$sum": {
                        "$cond": [
                            {"$eq": ["$financial_service_account.transaction.TRANSACTIONTYPE", sell_type]},
                            "$financial_service_account.transaction.VALUE",
                            0,
                        ]
                    }
    ...
    queries.append({
        ...
        "root_collection": "lmm_transaction",
        "pipeline": maybe_limit([
            {"$match": {"REFERSTO": security_id, "TRANSACTIONTYPE": sell_type}},
            {"$group": {
                "_id": "$REFERSTO",
                "avg_sell_value": {"$avg": "$VALUE"},
                "transactions": {
                    "$push": {
            ...
            {"$unwind": "$transactions"},
            {"$match": {"$expr": {"$lt": ["$transactions.VALUE", "$avg_sell_value"]}}},
        ], result_limit),
    })
    ...
    queries.append({
        ...
        "pipeline": maybe_limit([
            ...
            {"$group": {
                "_id": {
                    "person": "$PERSONID",
                    "stock": "$financial_service_account.transaction.REFERSTO",
                },
                "types": {"$addToSet": "$financial_service_account.transaction.TRANSACTIONTYPE"},
                "n_transactions": {"$sum": 1},
            }},
            {"$match": {"types": {"$all": [buy_type, sell_type]}}},
        ], result_limit),
    })
