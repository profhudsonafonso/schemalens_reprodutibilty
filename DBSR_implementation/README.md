...
## FIBEN SF1 DBSR vs SchemaLens result

The FIBEN SF1 comparison has been rerun after applying the same semantic-alignment protocol used for the final SF10 result.

Final protocol:

- MongoDB database: `dbsr_fiben_sf1_source_full`
- DBSR collections: full `dbsr_rank*` materialization
- Query set: FIBEN Q1–Q9
- Q10: excluded from read-query comparison because it is an insert/update workload
...
- DBSR wins: 2/9
- SchemaLens wins: 7/9
- DBSR near SchemaLens within 5%: 2/9
- Average DBSR regret vs SchemaLens: 7.531969

Main interpretation:

DBSR is competitive for Q1 and Q5, but SchemaLens is stronger across the full SF1 workload. The largest differences occur in associative/deep traversal queries such as Q3 and Q4, where returned-count parity confirms that the difference is physical-plan related rather than caused by parameter mismatch.

Main output files:

- `DBSR_implementation/results/fiben/dbsr_fiben_query_benchmark_aggregate_sf1_official_semantic_v9_original_pool_ibm2860_10hot.csv`
...
## FIBEN query-plan comparison: DBSR vs SchemaLens

The DBSR FIBEN query-plan analysis has been completed for SF1, SF10, and SF30.

The runner is:

- `DBSR_implementation/benchmark/fiben/run_dbsr_fiben_query_plan.py`

It reuses the same MongoDB `explain("executionStats")` criteria used by the SchemaLens FIBEN query-plan workflow:
...
- winning-plan stages
- index names
- `IXSCAN`, `COLLSCAN`, `FETCH`, `SORT`, `GROUP`, and related flags
- collection size and average object size
- estimated examined bytes

The DBSR query-plan outputs are stored in:

- `DBSR_implementation/results/fiben/query_plan/`

The comparison against SchemaLens best query-plan candidates is generated by:

- `DBSR_implementation/src/analysis/compare_dbsr_vs_schemalens_fiben_query_plan.py`

Main comparison outputs:

- `DBSR_implementation/results/fiben/query_plan/dbsr_vs_schemalens_fiben_query_plan_best_comparison.csv`
- `DBSR_implementation/results/fiben/query_plan/dbsr_vs_schemalens_fiben_query_plan_best_summary.csv`
- `DBSR_implementation/results/fiben/query_plan/dbsr_vs_schemalens_fiben_query_plan_focus_q3_q4_q5_q6.csv`
- `DBSR_implementation/results/fiben/query_plan/dbsr_vs_schemalens_fiben_query_plan_report.md`

Key finding:

- Q3 and Q4 explain the main SchemaLens advantage. DBSR preserves semantic alignment but examines many more keys/documents for traversal-heavy paths, especially at SF10 and SF30.
- Q5 and Q6 explain the main DBSR advantages. DBSR uses direct materialized access paths and examines far fewer documents/bytes for these cases.
